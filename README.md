...
<summary>Notes Before Usage</summary>
  ...
  publish:
    ...
    diagnostics: false
  ```
  In order to launch ROSaic, one must specify all `arg` fields of the `rover.launch` file which have no associated default values, i.e. for now only the `param_file_name` field. Hence, the launch command thus reads `roslaunch septentrio_gnss_driver rover.launch param_file_name:=rover`.
...
# Inertial Navigation System (INS): Basics
  ...
  - The IMU is typically made up of a 3-axis accelerometer, a 3-axis gyroscope and sometimes a 3-axis magnetometer and measures the system's angular rate and acceleration.
    ...
    + The IMU/antenna position can be changed by specifying the lever arm's `x`,`y`and `z` parameters in the `config.yaml` file under the `ins_spatial_config/ins_ant_lever_arm` parameter.
  ...
  - These Steps should be followed to configure the receiver in INS integration mode:
    ...
    - Specify the IMU-antenna lever arm in the vehicle reference frame. This is the vector starting from the IMU reference point to the ARP of the main GNSS antenna. This can be done by means of the `ins_spatial_config/ins_ant_lever_arm` parameter.
...
* Parameters Configuring Communication Ports and Processing of GNSS and INS Data
  ...
  + `poi_to_arp`: offsets of the antenna reference point (ARP) with respect to the point of interest (POI = marker)
